...
export class Firehose {
    ...
    async cmdReadBuffer(physicalPartitionNumber, startSector, numPartitionSectors) {
        ...
        try {
            // ---- QEFT patch ----
            // 原实现写死 2000ms，回读大分区（几十 MB 起）必然超时，且超时后
            // 流里残留半截 raw 数据导致后续命令全部错位。按保守 1MB/s 估算
            // 超时时间，下限仍为 2s。
            const bytes = this.cfg.SECTOR_SIZE_IN_BYTES * numPartitionSectors;
            const timeoutMs = Math.max(2000, Math.ceil((bytes / (1024 * 1024)) * 1000) + 1000);
            buffer = await runWithTimeout(this.cdc.read(bytes), timeoutMs);
        }
        catch {
    ...
    async cmdGetStorageInfo() {
        ...
        if (!resp.resp || !resp.log)
            throw new Error("Failed to get storage info", { cause: resp.error });
        return resp.log;
    }
    async cmdDeviceType() {

...
export class qdlDevice {
    ...
    get firehose() {
        if (!this.#firehose)
            throw new Error("Firehose not configured");
    ...
    async getStorageInfo() {
        ...
        if (!log)
            throw new Error("Storage info JSON not returned - not implemented?");
        try {
    ...
    async getDeviceType() {
        ...
        if (!deviceType)
            throw new Error("Device type not returned - not implemented?");
        return deviceType;
    }
    /**
     * @param {"a"|"b"} slot
